...
class SuperResolution:
    ...
    def load_datasets(self, target, data_dir, batch_dir, batch_image_size, stride_size=0):
        ...
        datasets = util.DataSets(self.scale, batch_image_size, stride_size, channels=self.channels,
    ...
    def build_input_batch(self, batch_dir):
        ...
        for i in range(self.batch_num):
            ...
            batch_input_quad = util.load_image(batch_dir + "/" + INTERPOLATED_IMAGE_DIR + "/%06d.bmp" % image_no,
                                               print_console=False)
            util.convert_to_multi_channel_image(self.batch_input_quad[i], batch_input_quad, self.scale)
            batch_true_quad = util.load_image(batch_dir + "/" + TRUE_IMAGE_DIR + "/%06d.bmp" % image_no,
                                              print_console=False)
            util.convert_to_multi_channel_image(self.batch_true_quad[i], batch_true_quad, self.scale)
    ...
    def do(self, input_image, bicubic_input_image=None):
        ...
        util.convert_to_multi_channel_image(quad_image[0], bicubic_image, self.scale)
        ...
        util.convert_from_multi_channel_image(image, quad_image, self.scale)
    ...
    def do_for_evaluate(self, file_path, output_directory="output", output=True, print_console=True):
        ...
        if true_image.shape[2] == 3 and self.channels == 1:
            input_y_image = util.build_input_image(true_image, channels=self.channels, scale=self.scale,
            ...
            input_image = util.build_input_image(true_image, channels=self.channels, scale=self.scale,
